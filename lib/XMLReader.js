...
const XMLReader = class extends Transform {
	...
	constructor (options = {}) {
		...
		this.attrsFilter  = options.attrsFilter
	...
	tag_open (name, attrs) {
		...
		let {stack} = this; if (stack.length === 0) {
			...
			if (this.attrsFilter && !this.attrsFilter (attrs)) return
	...
	parse_attrs (attrs) {
		...
		for (let i = 0; i < tokens.length - 1; i ++) {
			...
			if (ignore_namespaces || !/^xmlns/.test (k)) {
				...
				if (name) this._attr_map.set (name, tokens [i].trim () || null)
