...
const XMLReader = class extends Transform {
	...
	tag_open (name, attr) {
		...
		let {stack} = this; if (stack.length === 0 && this.localName) {
			...
			if (this.localName != localName) return
			
			if (this.use_namespaces && this.nameSpace && this.nameSpace != nameSpace) return

		}
		
		let fieldMap = stack.length === 0 ? this.fieldMap : null
	
		let kvs = this.attr_to_kv_pairs (attr, fieldMap)

		stack.push (new Element (this.rename (localName, nameSpace, false), kvs, fieldMap))

	}
	
	attr_to_kv_pairs (attr, fieldMap) {
	
		if (attr === null) return EMPTY_ARRAY
		
		let entries = Object.entries (attr); if (this.ignore_namespaces && !fieldMap) return entries
	
		let {use_namespaces} = this, kvs = []; for (let [k, v] of entries) {
				
			if (!use_namespaces && /^xmlns/.test (k)) continue // if use_namespaces, they are already deleted
			...
			let ln, ns = null, pos = k.indexOf (':'); if (pos < 0) {
				...
				ln = k.slice (pos + 1)
				if (use_namespaces) ns = this.nsStack.get (k.slice (0, pos))
			}
			
			let name = this.rename (ln, ns, true)
